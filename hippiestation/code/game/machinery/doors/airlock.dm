...
/obj/machinery/door/airlock/AltClick(mob/living/user)
	...
	for(var/mob/living/silicon/ai/AI in GLOB.ai_list)
		if(!AI.client)
			continue
		to_chat(AI, "<span class='info'><a href='?src=[REF(AI)];track=[rhtml_encode(user.name)]'><span class='name'>[user.name] ([user.GetJob()])</span></a> is requesting you to open [src]<a href='?src=[REF(AI)];remotedoor=[REF(src)]'>(Open)</a></span>")
